...
2. **Debouncing y Captura Manual Híbrida:** La cámara inyecta 30 fotogramas por segundo. Para evitar la inestabilidad de la interfaz, se implementó un algoritmo de *Debouncing* que exige múltiples fotogramas continuos para confirmar una predicción. Una vez que el objeto es estable en pantalla, el sistema solicita al usuario que presione el botón "Capturar". Este enfoque manual otorga al usuario el control absoluto sobre qué elementos registrar en la base de datos, evitando falsos positivos. Visualmente se apoya con una barra de progreso animada y acústicamente con el generador de tonos (`ToneGenerator`) de Android al confirmar la captura.
